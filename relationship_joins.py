...
DATABASE_URI = (
    "mysql://root:insecure@database:3306/relationship_joins?charset=utf8&use_unicode=1"
)
...
def monitor_queries(fn):
    def decorated(*args, **kwargs):
        ...
        LOG.debug(
            "--- EMITTED QUERIES for function call %s(%s, %s) --"
...
def get_user_companies():
    ...
    out = []
    for user in users:
        out.append({"name": user.name, "company_name": user.company.name})
    return out
        ...
        out.append(
            {
                "name": user.name,
                "emails": [
                    {"type": email.type, "email": email.email} for email in user.emails
                ],
            }
        )
    return out
...
def get_user_phones(phone_type=None):
    ...
    for user in users:
        ...
        if phone_type:
            phone_numbers = phone_numbers.filter(PhoneNumber.type == phone_type)
        out.append(
            {
                "name": user.name,
                "phones": [
